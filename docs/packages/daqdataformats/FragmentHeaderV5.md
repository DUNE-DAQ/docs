...
This document describes the format of the `FragmentHeader` struct, version 5. It should **not** be updated, but rather kept as a historic record of the data format for this version.

# FragmentHeader Description

Version 5 of the `FragmentHeader` consists of 18 32-bit words:
...
15. Sequence Number (upper 16 bits) / Detector ID (lower 16 bits)
...
# Changes from v4 to v5
 ...
 The only change from version 4 to version 5 of the `FragmentHeader` is that the lower 16 bits of word 15, which were previously unused padding, are now used to store a `detector_id` field.
