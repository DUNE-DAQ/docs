...
_n.b. If you want to build your repo area against Spack packages rather than UPS packages, please read [these instructions](README_spack.md)_
...
setup_dbt v4.1.7 # v4.1.7 corresponds to the dunedaq-v2.11.1 release
```
...
Running a release from cvmfs without creating a work area is supported since the `dunedaq-v2.8.1` release. To do that, simply run the following (&#x1F53A; Please note: &#x1F53A; there are two variants of the release for `dunedaq-v2.11.1`. The `<release>` field mentioned in this page should be either `dunedaq-v2.11.1-cs8` or `dunedaq-v2.11.1-c7`):

```sh
dbt-setup-release <release> # e.g. dbt-setup-release dunedaq-v2.11.1-cs8
```
...
Note that if you add a new repo to your work area, after building your new code - and hence putting its output in `./build` - you'll need to run `dbt-workarea-env` again. 
...
_Date: Tue Jun 7 09:20:08 2022 -0500_
