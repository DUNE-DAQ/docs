...
Running a release from cvmfs without creating a work area is supported since the `dunedaq-v2.8.1` release. To do that, simply run the following (&#x1F53A; Please note: &#x1F53A; there are two variants of the release for `dunedaq-v2.11.0`. The `<release>` field mentioned in this page should be either `dunedaq-v2.11.0-cs8` or `dunedaq-v2.11.0-c7`):

```sh
dbt-setup-release <release> # e.g. dbt-setup-release dunedaq-v2.11.0-cs8
```
...
_Date: Mon Jun 6 18:00:45 2022 -0500_
