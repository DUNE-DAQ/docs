...
- Analogous to how stable and candidate releases are made, the nightly release's packages are defined via `release.yaml` files in `configs/coredaq/coredaq-develop`,`configs/fddaq/fddaq-develop` and `configs/nddaq/nddaq-develop`
